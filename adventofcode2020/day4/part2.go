...
func main() {
	...
	var pspts = strings.Split(RawInput, "\n\n")
	re := regexp.MustCompile(`(\w{3}):([#\w\d]+)`)
	...
	count := 0
	for _, pp := range pspts {
		go func(pp string) {
			defer wg.Done()
			res := re.FindAllStringSubmatch(pp, -1)
			validator := makeValidator()
			isValid := true
			for _, match := range res {
				isValid = validator[match[1]](match[2])
				delete(validator, match[1])
			...
			if _, found := validator["cid"]; len(validator) == 0 || (len(validator) == 1 && found) {
				results <- true
			}
		}(pp)
	}
...
func makeValidator() map[string]func(string) bool {

	validator := map[string]func(string) bool{
		"byr": func(input string) bool {
			return validateYr(input, 1920, 2002)
		},
		"iyr": func(input string) bool {
			return validateYr(input, 2010, 2020)
		},
		"eyr": func(input string) bool {
			return validateYr(input, 2020, 2030)
		},
		"hgt": func(input string) bool {
			//fmt.Println(strings.HasSuffix(input, "cm"))
			if strings.HasSuffix(input, "cm") {
				cm, _ := strconv.Atoi(strings.TrimSuffix(input, "cm"))
				//fmt.Println(cm)
				return cm >= 150 && cm <= 193
			}
			if strings.HasSuffix(input, "in") {
				inch, _ := strconv.Atoi(strings.TrimSuffix(input, "in"))
		...
		"hcl": func(input string) bool {
			...
			matched, _ := regexp.Match("^[0-9a-f]{6}$", []byte(input[1:]))
			return matched
		},
		...
		"pid": func(input string) bool {
			matched, _ := regexp.Match("^[0-9]{9}$", []byte(input))
			return matched
		},
...
func validateYr(input string, min, max int) bool {
	if len(input) != 4 {
		return false
	}
	yr, _ := strconv.Atoi(input)
	return yr >= min && yr <= max
}

const RawInput = `ecl:amb
pid:690616023
