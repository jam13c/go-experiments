...
func main() {
	...
	board := newBoard(len(input[0]), len(input))
	for y, row := range input {
		for x, c := range row {
			board.p[x][y] = position{board, x, y, c}
		}
	}
	fmt.Printf("%v", board)
	changed := false
	...
	fmt.Printf("Num occupied = %v", board.numOccupied())
}
...
func newBoard(w, h int) *board {
	fmt.Printf("Making new board w=%d h=%v\n", w, h)
	p := make([][]position, w)
	for i := 0; i < w; i++ {
		p[i] = make([]position, h)
	}
...
func (board *board) doMove() (bool, *board) {
	nb := newBoard(board.w, board.h)
	changed := false
	for y := 0; y < board.h; y++ {
		for x := 0; x < board.w; x++ {
			p := &board.p[x][y]
			numOcc := p.numAdjacentOccupied()
			if p.c == empty && numOcc == 0 {
				changed = true
				nb.p[x][y] = position{nb, x, y, occupied}
			} else if p.c == occupied && numOcc >= 5 {
				changed = true
				nb.p[x][y] = position{nb, x, y, empty}
			} else {
				nb.p[x][y] = position{nb, x, y, p.c}
			}
		}
	}
	return changed, nb
...
func (board *board) numOccupied() int {
	occ := 0
	for y := 0; y < board.h; y++ {
		for x := 0; x < board.w; x++ {
			if board.p[x][y].c == occupied {
				occ++
			}
		}
	}
...
func (pos *position) numAdjacentOccupied() int {
	adj := [8]adjust{
		{-1, -1},
		{-1, 0},
		{-1, 1},
	...
	for i := 0; i < len(adj); i++ {
		x := pos.x + adj[i].x
		y := pos.y + adj[i].y
		for x >= 0 && x < pos.board.w && y >= 0 && y < pos.board.h {
			p := pos.board.p[x][y]
			if p.c == occupied {
				occ++
				break
...
const DemoInput2 = `LLLLLLLLLLLL.LLLLLLLLLL.LLLL.LLLL.LLLLLLLLLLL.LLLL.LLLLLLL.LL.LLLLLLLL.LLL.LLLLLLLLLLL.LLLLLLL
...
const RawInput = `LLLLLLLLLLLL.LLLLLLLLLL.LLLL.LLLL.LLLLLLLLLLL.LLLL.LLLLLLL.LL.LLLLLLLL.LLL.LLLLLLLLLLL.LLLLLLL
LLLLLLLLLLL.LLLL.L..LLL.LL.LLLLLLLLLLL.LLLLLL.LLLLLLLL.L.LLLLL.LLLLLL..LLL.LLLLLLLLLLLLLLLLLLL
LLLLLL.LLLL.LLLL.L.LLLL.LLL.LLLLLLLLLLLLLLLLL.LLLLLLLLLLLLLLL..LLLLLLLLLLLLLLLLLLLLLLLLLLL.LLL
LLLLLLLLLLL.LL.LL..LLLL.LLLL.L.LLLLLLL.LLLLLL.LLLLLLLLLLLLLLL.LLLLLLLL.LLLLLLLL.LLLLLLLLLLLLLL
