...
func Positions(input []string) int {
	re := regexp.MustCompile(`(?P<pos1>\d+)-(?P<pos2>\d+)\s(?P<Char>\w{1}):\s(?P<Password>\w+)`)
	numValid := 0
	for _, in := range input {
		res := re.FindStringSubmatch(in)
		p1, _ := strconv.Atoi(res[1])
		p2, _ := strconv.Atoi(res[2])
		if isValidPos(p1, p2, rune(res[3][0]), res[4]) {
			numValid++
		}
...
func isValidPos(p1, p2 int, char rune, password string) bool {
	if (rune(password[p1-1]) == char || rune(password[p2-1]) == char) && rune(password[p1-1]) != rune(password[p2-1]) {
		return true
	}
	return false
}

func MinMax(input []string) {
	re := regexp.MustCompile(`(?P<Min>\d+)-(?P<Max>\d+)\s(?P<Char>\w{1}):\s(?P<Password>\w+)`)
	numValid := 0
	for _, in := range input {
		res := re.FindStringSubmatch(in)
		min, _ := strconv.Atoi(res[1])
		max, _ := strconv.Atoi(res[2])
		if isValidMinMax(min, max, rune(res[3][0]), res[4]) {
			numValid++
		}
...
func isValidMinMax(min, max int, char rune, password string) bool {
	count := 0
