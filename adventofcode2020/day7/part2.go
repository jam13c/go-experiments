...
func main() {
	rules := getRules()
	//bags := make([]bag, 0)
	count := howManyBags(rules, "shiny gold")

	fmt.Println(count)
}

func howManyBags(theRules rules, theBag bag) int {
	return recursiveCount(theRules, theBag) - 1
}

func recursiveCount(theRules rules, theBag bag) int {
	count := 1
	for subBag, subBagCount := range theRules[theBag] {
		count += subBagCount * recursiveCount(theRules, subBag)
	}
	return count
}
...
const DemoInput2 = `shiny gold bags contain 2 dark red bags.
