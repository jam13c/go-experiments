...
func main() {
	rules := getRules()
	bags := make([]bag, 0)
	rules.recursiveFind("shiny gold", &bags)

	fmt.Println(len(bags))
}

func (theRules *rules) recursiveFind(theBag bag, bags *[]bag) {
	foundBags := theRules.findWhatCanHold(theBag)
	if len(foundBags) == 0 {
		return
	}
	for _, foundBag := range foundBags {
		if !slice.Contains(*bags, foundBag) {
			*bags = append(*bags, foundBag)
			theRules.recursiveFind(foundBag, bags)
		}
	}
	return
}

func (theRules *rules) findWhatCanHold(theBag bag) []bag {
	localBags := make([]bag, 0)
	for k, v := range *theRules {
		if _, found := v[theBag]; found {
			localBags = append(localBags, k)
		}
	}
	return localBags
}
...
func getRules() rules {
	re := regexp.MustCompile(`(?m)^(\w+\s\w+)\sbags\scontain\s([\w\d,\s]+).$`)
	bagre := regexp.MustCompile(`\s*(\d+)\s(\w+\s\w+)\sbags?`)

	matches := re.FindAllStringSubmatch(RawInput, -1)

	rules := make(map[bag]map[bag]int)
	for _, match := range matches {
		top := bag(match[1])
		bagMatches := bagre.FindAllStringSubmatch(match[2], -1)

		containers := make(map[bag]int)
		for _, bagMatch := range bagMatches {
			count, _ := strconv.Atoi(bagMatch[1])

			containers[bag(bagMatch[2])] = count
		}

		rules[top] = containers
...
const RawInput = `dull silver bags contain 2 striped magenta bags, 2 dark coral bags, 1 bright orange bag, 4 plaid blue bags.
