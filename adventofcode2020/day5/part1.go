...
func main() {
	...
	boardingPasses := strings.Split(RawInput, "\n")

	var wg sync.WaitGroup
	wg.Add(len(boardingPasses))

	resultChan := make(chan int)
	max := 0

	for i := 0; i < len(boardingPasses); i++ {
		go func(bp string) {
			defer wg.Done()
			resultChan <- seatID(traverse(bp))
		}(boardingPasses[i])
	}
