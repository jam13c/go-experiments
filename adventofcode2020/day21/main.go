...
func main() {
	input, allergens := parseInput(RawInput)
	//part1(input, allergens)
	part2(input, allergens)
}

func part1(input []inputData, allergens []string) {

	actualAllergens := findAllergens(input, allergens)
	count := 0
	for _, row := range input {
		for i := range row.ingredient {
			if _, found := actualAllergens[i]; !found {
				count++
			}
		}
	}
...
func part2(input []inputData, allergens []string) {
	actualAllergens := findAllergens(input, allergens)
	...
	sort.Slice(data, func(x, y int) bool {
		return data[x].alle < data[y].alle
	})

	ingredients := make([]string, 0)
	for _, i := range data {
		ingredients = append(ingredients, i.ing)
	}
	fmt.Println(strings.Join(ingredients, ","))
}

func findAllergens(input []inputData, allergens []string) map[string]string {
	possibleAllergens := make(map[string]map[string]bool)
	for _, alle := range allergens {
		possibleAllergens[alle] = findPossibleAllergen(alle, input)
	}
		...
		for allergen, ingredients := range possibleAllergens {
			if len(ingredients) == 1 {
				for i := range ingredients {
					actualAllergens[i] = allergen
					for otherAllergen, otherIngredients := range possibleAllergens {
						for range otherIngredients {
							delete(possibleAllergens[otherAllergen], i)
						}
					}
				}
...
func findPossibleAllergen(allergen string, input []inputData) map[string]bool {

	foundRows := make([]inputData, 0)
	for _, row := range input {
		if _, found := row.allergen[allergen]; found {
			foundRows = append(foundRows, row)
		}
	}
	return findIntersectingIngredients(foundRows)
}

func findIntersectingIngredients(input []inputData) map[string]bool {
	m1 := input[0].ingredient
	for _, row := range input[1:] {
		m1 = intersect(m1, row.ingredient)
	}
	return m1
}

func intersect(m1 map[string]bool, m2 map[string]bool) (result map[string]bool) {
...
func parseInput(input string) (inData []inputData, allAllergens []string) {
	re := regexp.MustCompile(`([\w\s]+)\s\(contains\s([\w\s,]+)\)`)
	lines := strings.Split(input, "\n")
	a := map[string]bool{}
	for _, line := range lines {
		m := re.FindStringSubmatch(line)
		ing := sliceToMap(strings.Split(m[1], " "))
		alle := sliceToMap(strings.Split(m[2], ", "))
		for k := range alle {
			a[k] = true
		}
		inData = append(inData, inputData{ing, alle})
	}
	for k := range a {
		allAllergens = append(allAllergens, k)
	}
	return
}

func sliceToMap(in []string) map[string]bool {
	m := map[string]bool{}
	for _, s := range in {
		m[s] = true
	}
...
type inputData struct {
	ingredient map[string]bool
	allergen   map[string]bool
}

const DemoInput = `mxmxvkd kfcds sqjhc nhms (contains dairy, fish)
trh fvjkl sbzzf mxmxvkd (contains dairy)
sqjhc fvjkl (contains soy)
sqjhc mxmxvkd sbzzf (contains fish)`

const RawInput = `xnjhn pgmj lclnj qgnpkp vpfstj ffdsnc mnpfcx tkccrc lkv cgdx rmfmm grjrrm frq lvzjpn vntgs cbzcgvc mnnzf rfdm vxrdlb dvmzkk vpsmz sfp kltxt sxbr dlrpp pqrvc lpqxmc kfgln zzvhml prgnr svpfz scfhknrs nvzjxr tpv jmvxx nxkpfqtg lgtpk vcrs pqqks dxxmnr jlxqpjd qp mjhdfg vbbk qtsjztd tnfgxsn zhxh rrmvn tnn gdfddn jx tbv vnddzb (contains soy, peanuts, sesame)
